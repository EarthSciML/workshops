...
# ╔═╡ c2d3e4f5-0003-0003-0003-000000000100
begin
...
# ╔═╡ c2d3e4f5-0003-0003-0003-000000000101
@register_unit ppb 1

# ╔═╡ c2d3e4f5-0003-0003-0003-000000000103
md"""
# Session 3: Boundary Layer Depth and Stability Index

**Reference**: Middleton (1998), Sections 4–6

---

In Session 2 we coupled the box model to the sensible heat flux, but the mixing height *h* was still fixed at 400 m. In this session we add the **boundary layer depth** model, which makes *h* vary diurnally — deep during the day (strong mixing, low pollution) and shallow at night (poor mixing, high pollution). This is the single biggest driver of the diurnal concentration cycle.
"""

# ╔═╡ c2d3e4f5-0003-0003-0003-000000000104
md"""
## Learning Objectives

After completing this notebook, you will be able to:

- Implement **friction velocity** u\*, **Coriolis parameter** f, and **boundary layer depth** zᵢ as ModelingToolkit components with physical units (Middleton Eqs. 6–10)
- Implement the **Pasquill stability index** P as a ModelingToolkit component (Eqs. 11–12)
- Write **coupling equations** connecting the boundary layer depth to the heat flux and box model
- Explore how **latitude** affects estimates of boundary layer depth through the **Coriolis parameter**
- Compare three successive model versions against OpenAQ observations
"""

# ╔═╡ c2d3e4f5-0003-0003-0003-000000000110
md"""
The model components from Sessions 1 and 2 (BoxModelNO2, SolarRadiation, CloudCorrection, SensibleHeatFlux, and their coupling equations) are loaded in the **Appendix** at the end of this notebook.
"""

# ╔═╡ c2d3e4f5-0003-0003-0003-000000000200
md"""
## 1. Boundary Layer Depth (Eqs. 6–10)

The **atmospheric boundary layer** (ABL) is the lowest part of the atmosphere and is directly influenced by the Earth's surface through friction, heating, and moisture exchange. It typically ranges from ~100 m deep at night (when the surface cools and turbulence is suppressed) to ~1–2 km during the day (when solar heating drives strong convective mixing). For air quality, the ABL depth is critical because it determines the volume of air into which pollutants are mixed: a deeper boundary layer dilutes pollution, while a shallow one traps it near the surface.

### Friction velocity u\* (Eq. 6)

**Friction velocity** u\* measures the intensity of turbulent stress at the surface:

```math
u_* = \frac{\kappa \, u}{\ln(\Delta z / z_0)}, \quad \kappa = 0.4
```

where $\Delta z = 9.9$ m, $z_0$ is the surface roughness length (rural: 0.1 m, suburban: 0.3 m, city centre: 1.0 m), and κ is the von Kármán constant.

### Coriolis parameter f and neutral depth zₙ (Eq. 7)

```math
f = 2\Omega\sin\varphi, \quad z_n = \frac{0.25 \, u_*}{f}
```

where $\Omega = 7.2921 \times 10^{-5}$ rad s⁻¹ is the Earth's angular velocity and $\varphi$ is the latitude in radians.

| City | Latitude | f (s⁻¹) | Relative zₙ |
|:---|:---:|:---:|:---:|
| Mumbai | 19°N | 4.74 × 10⁻⁵ | 2.4× deeper |
| London | 51°N | 1.13 × 10⁻⁴ | 1× (reference) |

### Stable depth (Eqs. 8–9, H < 0)

```math
z_s = \frac{21500 \, u_*^2}{|H|^{1/2}}, \quad z_i = \left(\frac{1}{z_n^3} + \frac{1}{z_s^3}\right)^{-1/3}
```

where $H$ is the sensible heat flux (W m⁻²), $u_*$ is the friction velocity (m s⁻¹), $z_s$ is the stable boundary layer depth (m), and $z_n$ is the neutral boundary layer depth (m). Under stable conditions ($H < 0$), turbulence is generated mechanically by wind shear ($u_*$) but suppressed by negative buoyancy (cooling surface). The combined depth $z_i$ joins the neutral and stable estimates so that the shallower constraint dominates.

### Unstable depth (Eq. 10, H > 0)

```math
z_i = \sqrt{z_n^2 + \frac{1400 \, S_i}{\rho \, c_p}}
```

where $z_i$ is the boundary layer depth (m), $z_n$ is the neutral boundary layer depth (m), $S_i = \int H \, dt$ is accumulated sensible heat flux since dawn (J m⁻²), $\rho$ is the air density (kg m⁻³), and $c_p$ is the specific heat of air at constant pressure (J kg⁻¹ K⁻¹), with $\rho c_p \approx 1235$ J m⁻³ K⁻¹. When $H$ becomes negative (sunset), $S_i$ decays rapidly toward zero via exponential decay (with a 60-second timescale) for numerical stability, rather than resetting instantaneously.

### 📟 Exercise 1: Implement the boundary layer depth component

**Your task**: Replace each `missing` in the `BoundaryLayerDepth` component below with the correct equation. There are 6 equations to fill in.

**Variable mapping** — the code uses these names for the math symbols above:

| Math symbol | Code variable | Description |
|:---:|:---|:---|
| $u$ | `u_wind` | Wind speed at 10 m (m s⁻¹) |
| $u_*$ | `u_star` | Friction velocity (m s⁻¹) |
| $\kappa$ | `κ` | von Kármán constant (0.4) |
| $\Delta z$ | `Δz` | Height difference for log profile (9.9 m) |
| $z_0$ | `z0` | Surface roughness length (m) |
| $\Omega$ | `Ω` | Earth's angular velocity (rad s⁻¹) |
| $\varphi$ | `lat * deg_to_rad` | Latitude converted to radians |
| $f$ | `f` | Coriolis parameter (s⁻¹) |
| $z_n$ | `zn` | Neutral boundary layer depth (m) |
| $z_s$ | `zs` | Stable boundary layer depth (m) |
| $z_i$ | `zi` | Boundary layer depth (m) |
| $H$ | `H` | Sensible heat flux (W m⁻²) |
| $S_i$ | `S_i` | Accumulated heat flux since dawn (J m⁻²) |

**Hints for the trickier equations:**

- **Eq. 6 (`u_star`)**: Use Julia's `log()` function for the natural logarithm.
- **Eq. 7 (`f`)**: Remember to convert latitude from degrees to radians using `deg_to_rad`.
- **Eq. 7 (`zn`)**: The coefficient 0.25 is available as the constant `zn_coeff`.
- **Eq. 8 (`zs`)**: The coefficient 21500 is available as `c_stable`. Use `max(abs(H), H_min)` instead of bare `|H|` to avoid division by zero when H is near zero. Use `(1 // 2)` for the exponent ½.
- **`D(S_i)`** (no equation number in the paper): This tracks accumulated solar heating since dawn. Use `ifelse(H > H_zero, H, -S_i / τ_reset)` — when H > 0 (daytime), accumulate heat; when H ≤ 0 (night), let S_i decay exponentially with timescale `τ_reset`.
- **Eqs. 9–10 (`zi`)**: Use `ifelse` to switch between unstable (H > 0, Eq. 10) and stable (H < 0, Eq. 9). For the unstable case, the coefficient 1400 is `c_unstable`, and divide by `(ρ * c_p)` to convert to kinematic units. Use `max(S_i, S_zero)` to prevent negative values under the square root. For the stable case, the formula $(1/z_n^3 + 1/z_s^3)^{-1/3}$ can be rewritten as `(zn^3 * zs^3 / (zn^3 + zs^3))^(1 // 3)` (which is more numerically stable).

The constants `H_zero` and `S_zero` are zero-valued with appropriate units, used for comparisons inside `ifelse`. `H_min` is a small positive heat flux used as a floor to prevent division by zero.
"""

# ╔═╡ c2d3e4f5-0003-0003-0003-000000000202
begin
	struct BoundaryLayerDepthCoupler; sys; end
	@component function BoundaryLayerDepth(; name=:BoundaryLayerDepth)
		@constants begin
			...
			c_stable = 21500.0, [description="Stable BLD coefficient", unit=u"kg^(1//2)*s^(1//2)/m"]
			c_unstable = 1400.0, [description="Unstable BLD coefficient", unit=u"m/K"]
			ρ = 1.229, [description="Air density", unit=u"kg/m^3"]
			c_p = 1005.0, [description="Specific heat of air", unit=u"J/kg/K"]
			H_min = 1.0e-3, [description="Minimum heat flux", unit=u"W/m^2"]
			H_zero = 0.0, [description="Zero heat flux", unit=u"W/m^2"]
			S_zero = 0.0, [description="Zero accumulated flux", unit=u"J/m^2"]
		...
		eqs = [
			u_star ~ missing, # Eq. 6: friction velocity
			f ~ missing, # Eq. 7: Coriolis parameter
			zn ~ missing, # Eq. 7: neutral BLD
			zs ~ missing, # Eq. 8: stable BLD
			D(S_i) ~ missing, # accumulate when H>0, reset when H≤0
			zi ~ missing, # Eqs. 9-10: combined BLD
		]
		System(eqs, t; name,
...
# ╔═╡ c2d3e4f5-0003-0003-0003-000000000205
let
    if !@isdefined(BoundaryLayerDepth)
        not_defined(:BoundaryLayerDepth)
    else
        try
            # Compute reference values from known equations
            κ = 0.4; Ω = 7.2921e-5; lat = 19.0; z0 = 0.1; Δz = 9.9; u_w = 3.0; H_val = -50.0
            ustar_ref = κ * u_w / log(Δz / z0)
            f_ref = 2 * Ω * sin(lat * π / 180)
            zn_ref = 0.25 * ustar_ref / f_ref
            zs_ref = 21500.0 * ustar_ref^2 / max(abs(H_val), 1.0e-3)^0.5
            zi_ref = (zn_ref^3 * zs_ref^3 / (zn_ref^3 + zs_ref^3))^(1 / 3)

            bl = BoundaryLayerDepth()
            sys = mtkcompile(bl)
            prob = ODEProblem(
                sys, [sys.H => H_val, sys.u_wind => u_w], (0.0, 1.0),
            )
            sol = solve(prob, Tsit5())
            zi_stable = sol[sys.zi][1]
            ustar_val = sol[sys.u_star][1]
            if ismissing(zi_stable) || !isfinite(zi_stable) || zi_stable <= 0
                keep_working(md"The stable BLD returned a non-positive or non-finite value. Check Eqs. 6–9.")
            elseif !isfinite(ustar_val) || ustar_val <= 0
                keep_working(md"Friction velocity u\* is non-positive. Check Eq. 6.")
            elseif !(zi_stable ≈ zi_ref)
                keep_working(md"Your stable-case BLD doesn't match the expected value. Check your equations for u_star, zn, zs, and zi.")
            else
                correct(md"BoundaryLayerDepth works! u\* = $(round(ustar_val, digits=3)) m/s, zᵢ (stable) = $(round(zi_stable, digits=1)) m.")
            end
        catch e
...
# ╔═╡ c2d3e4f5-0003-0003-0003-0000000002f0
md"""
Atmospheric stability is a key control on air quality because it determines how rapidly pollutants are mixed vertically away from the surface. In unstable conditions (strong solar heating), vigorous convection dilutes emissions through a deep boundary layer; in stable conditions (nighttime cooling), vertical mixing is suppressed and pollutants accumulate near the ground. The **Pasquill stability index** P provides a practical bridge between the physical drivers we have already modeled -- sensible heat flux H and wind speed u -- and the vertical dispersion parameter $\sigma_z$ that we will use in Notebook 4 to predict downwind pollutant concentrations from a Gaussian plume.
"""

# ╔═╡ c2d3e4f5-0003-0003-0003-000000000300
md"""
## 2. Stability Index P (Eqs. 11–12)

The **Pasquill stability class** summarises the turbulent state of the atmosphere on a continuous 1–7 scale:

| P | Letter | Description |
|:---:|:---:|:---|
| 1 | A | Very unstable — strong convection |
| 3.6 | D | Neutral — wind-dominated mixing |
| 7 | G | Very stable — inversion, fog possible |

Class A (strong instability) corresponds to strong sunshine with light winds; Class D (neutral) to overcast skies with moderate winds; Classes F-G (strong stability) to clear nights with light winds.

BOXURB computes P as a continuous variable based on H and u.

**Unstable (H > 0):**
```math
P = 7 - \left[2.26 + 0.019(U_s-5.6)^2\right]\left[0.1H + 2 + 0.4U_s^{3/2}\right]^{0.28-0.004(U_s-2)^2}
```

**Stable (H ≤ 0):**
```math
P = 3.6 + 10^{0.08-0.025U_s^2}\left|0.1H\right|^{1.54-0.011U_s^2}
```

where $U_s = \min(u, 8)$ m/s.

Note that the stability index P does not yet affect the box model concentration in this session — it will drive the vertical dispersion parameter σ_z in Notebook 4, which determines the effective mixing height.

### 📟 Exercise 2: Implement the stability index component

**Your task**: Replace the `missing` in the `P` equation with the correct expression implementing Eqs. 11 and 12. Use `ifelse(H/H_ref > 0, unstable_expr, stable_expr)`. While we usually want to make sure to always use units in our equations to avoid errors, in this case because this is an empirical equation whith many non-physical parameters, we will divide all physical quantities by corresponding reference values (`u_ref`, `H_ref`) to keep things dimensionless.
"""

# ╔═╡ c2d3e4f5-0003-0003-0003-000000000302
begin
	struct StabilityIndexCoupler; sys; end
	@component function StabilityIndex(; name=:StabilityIndex)
		@constants begin
			u_cap = 8.0, [description="Wind speed cap", unit=u"m/s"]
		...
		eqs = [
			Us ~ min(u, u_cap),
			P ~ missing, # Exercise 2: implement Eqs. 11-12
		]
		System(eqs, t; name,
...
# ╔═╡ c2d3e4f5-0003-0003-0003-000000000304
details(
    "Reference implementation (reveal after attempting)", Markdown.parse(
        ...
        P ~ ifelse(H/H_ref > 0,
            ...
            3.6 + 10.0^(0.08 - 0.025*(Us/u_ref)^2) *
                  abs(0.1*H/H_ref + 1e-9)^(1.54 - 0.011*(Us/u_ref)^2))
        ```
        """
    )
)

# ╔═╡ c2d3e4f5-0003-0003-0003-000000000305
let
    if !@isdefined(StabilityIndex)
        not_defined(:StabilityIndex)
    else
        try
            # Compute reference P values from known formulas
            # Unstable: H=250, u=4
            Us1 = min(4.0, 8.0)
            P_unstable_ref = 7.0 - (2.26 + 0.019 * (Us1 - 5.6)^2) *
                (0.1 * 250 + 2.0 + 0.4 * Us1^1.5)^(0.28 - 0.004 * (Us1 - 2.0)^2)
            # Stable: H=-26, u=1
            Us2 = min(1.0, 8.0)
            P_stable_ref = 3.6 + 10.0^(0.08 - 0.025 * Us2^2) *
                abs(0.1 * (-26.0) + 1.0e-9)^(1.54 - 0.011 * Us2^2)

            si = StabilityIndex()
            sys = mtkcompile(si)
            prob1 = ODEProblem(
                sys, [sys.H => 250.0, sys.u => 4.0], (0.0, 1.0),
            )
            sol1 = solve(prob1, Tsit5())
            P_unstable = sol1[sys.P][1]
            prob2 = ODEProblem(
                sys, [sys.H => -26.0, sys.u => 1.0], (0.0, 1.0),
            )
            sol2 = solve(prob2, Tsit5())
            P_stable = sol2[sys.P][1]

            if ismissing(P_unstable)
                still_missing(md"Fill in the P equation in the StabilityIndex component.")
            elseif !(P_unstable ≈ P_unstable_ref) || !(P_stable ≈ P_stable_ref)
                keep_working(md"P values don't match the expected results. Check your implementation of Eqs. 11–12, including the wind speed cap and the use of reference values.")
            else
                correct(md"StabilityIndex works! P = $(round(P_unstable, digits=2)) (unstable, H=250) and P = $(round(P_stable, digits=2)) (stable, H=−26).")
            end
        catch e
            if e isa ModelingToolkitBase.ValidationError
                keep_working(md"The system contains a unit mismatch. This can happen if your equations are incorrect, or if you haven't yet replaced the placeholder values.")
...
# ╔═╡ c2d3e4f5-0003-0003-0003-000000000400
md"""
## 3. Coupling the New Components

Now that we have both new components defined as standalone systems, we need to wire them into our existing model so that the boundary layer depth and stability class respond to the heat flux. The boundary layer depth depends on the sensible heat flux *H* and wind speed, and it outputs the mixing height *h* that the box model and heat flux model both need.

### 📟 Exercise 3: Write the coupling equations

**Your task**: Write three `couple2` methods:

1. **BoundaryLayerDepth ↔ BoxModelNO2**: Convert `h` in BoxModelNO2 to a variable, then connect `bm.h` to `bl.zi`. Also connect wind speed: convert `u_wind` in BoundaryLayerDepth to a variable and set `bl.u_wind` equal to `bm.u_wind`.

2. **BoundaryLayerDepth ↔ SensibleHeatFlux**: Convert `H` in BoundaryLayerDepth to a variable and connect `bl.H` to `hf.H`. Convert `h` in SensibleHeatFlux to a variable and connect `hf.h` to `bl.zi`.

3. **StabilityIndex ↔ SensibleHeatFlux**: Convert `H` and `u` in StabilityIndex to variables, then connect `si.H` to `hf.H_urban` and `si.u` to `hf.u` so the stability class tracks the current heat flux and wind speed. Note that the stability index uses the *urban* heat flux (`H_urban`), not the rural heat flux (`H`). The boundary layer depth is driven by the regional-scale (rural) heat flux because it represents the large-scale atmospheric response, while the stability index uses the urban heat flux because it characterizes local turbulence conditions within the city where pollutant concentrations are being modeled.
"""

# ╔═╡ c2d3e4f5-0003-0003-0003-000000000500
md"""
## 4. Full Coupled Model

### 📟 Exercise 4: Couple the full model and solve

**Your task**: Use `couple()` to combine all seven components — `BoxModelNO2()`, `SolarRadiation()`, `CloudCorrection()`, `SensibleHeatFlux()`, `BoundaryLayerDepth()`, `StabilityIndex()`, and the `OpenAQ` observations — into a single coupled system. Then convert, build the ODE problem, and solve.

The `OpenAQ` component (from the EarthSciData package) provides hourly observed NO₂ concentrations from OpenAQ monitoring stations in the model domain, which we use to compare against our model predictions. (OpenAQ is a global open-data platform that aggregates government air quality measurements from monitoring stations worldwide, including stations in Mumbai.)
"""

# ╔═╡ c2d3e4f5-0003-0003-0003-000000000502
full3_coupled = couple(
	missing # Exercise 4: couple all seven components
)

# ╔═╡ 6bf103b3-22e7-4ad0-9659-f7b6cb172aea
md"This is what our coupled model looks like now:"

# ╔═╡ aacab645-d37a-4359-a4fd-154151c4791e
let
	g = graph(full3_coupled)
...
# ╔═╡ 0a5c5369-3185-4ec3-93f3-dcd1b3623e4b
md"""
...and this is what it looks like when we solve it:
"""

# ╔═╡ c2d3e4f5-0003-0003-0003-000000000509
nothing

# ╔═╡ c2d3e4f5-0003-0003-0003-000000000505
nothing

# ╔═╡ c2d3e4f5-0003-0003-0003-000000000600
md"""
## 5. Interactive Exploration

Use the controls below to explore how wind speed and city latitude shape the diurnal boundary layer depth and stability index. Switch between **Mumbai** and **London** to observe the Coriolis effect directly.

| Parameter | Control | Units |
|:---|:---|:---:|
| Wind speed u: | $(@bind u_bl_slider PlutoUI.Slider(1.0:0.5:10.0, default=4.0, show_value=true)) | m/s |
| City / latitude: | $(@bind lat_selector PlutoUI.Select([19.0 => "Mumbai 19°N", 51.0 => "London 51°N"], default=19.0)) | — |
"""

# ╔═╡ c2d3e4f5-0003-0003-0003-000000000506
md"""
### 👉 Question 1: Coriolis Effect on Boundary Layer Depth

The neutral boundary layer depth is $z_n = 0.25\,u_*/f$, where $f = 2\Omega\sin\varphi$ is the Coriolis parameter. Physically, the boundary layer grows upward until the Coriolis effect deflects the wind enough to suppress further vertical mixing. Explain **why** a weaker Coriolis effect (smaller $f$) allows the boundary layer to grow deeper before this rotational constraint kicks in. Then compare Mumbai (19°N, $f = 4.74 \times 10^{-5}$ s⁻¹) to London (51°N, $f = 1.13 \times 10^{-4}$ s⁻¹): for the same wind speed, how much deeper is Mumbai's neutral BLD, and what does this imply for daytime pollutant concentrations?
"""

# ╔═╡ c2d3e4f5-0003-0003-0003-000000000507
hint(
    md"""
    Consider how the Coriolis parameter $f$ depends on latitude, and how $f$ appears in the formula for $z_n$. Think about what the Coriolis force does to the wind profile as you go higher in the atmosphere, and what that implies for the depth of the boundary layer.
    """
)

# ╔═╡ c2d3e4f5-0003-0003-0003-000000000508
answer_q1 = """
Enter your answer here.

""";

# ╔═╡ c2d3e4f5-0003-0003-0003-000000000603
md"""
### 👉 Question 2: Winter Night vs. Summer Noon Concentrations

Using the box model formula $C = C_{\text{bg}} + q \cdot x \cdot \chi / (h \cdot u)$, estimate the **ratio** of winter-night NO₂ to summer-noon NO₂ concentrations in Mumbai.

Use these scenarios:
- **Summer noon**: H = 250 W/m², u = 4 m/s, zᵢ ≈ deep (from the interactive plot above)
- **Winter night**: H = −26 W/m², u = 1 m/s, zᵢ ≈ shallow (stable BLD)

Think about which variables in the box model formula differ between these two scenarios and which cancel out in the ratio.
"""

# ╔═╡ c2d3e4f5-0003-0003-0003-000000000604
hint(
    md"""
    Think about how boundary layer depth and wind speed differ between these scenarios and how they affect concentration.
    """
)

# ╔═╡ c2d3e4f5-0003-0003-0003-000000000605
answer_q2 = """
Enter your answer here.

""";

# ╔═╡ c2d3e4f5-0003-0003-0003-000000000700
md"""
## 6. Model Comparison: Three Sessions vs. Observations

Now let's compare the three model versions we've built across Sessions 1–3:

1. **Session 1** — Box model only (constant h = 400 m, constant T)
2. **Session 2** — Box model + sensible heat flux (constant h = 400 m, varying T)
3. **Session 3** — Full model with boundary layer depth (varying h, varying T)

All three are compared against **OpenAQ** NO₂ observations for Mumbai, Nov 15–22, 2021.
"""

# ╔═╡ c2d3e4f5-0003-0003-0003-000000000704
md"""
The Session 3 model captures the **diurnal cycle** of NO₂ that Sessions 1 and 2 missed. The key improvement is the time-varying mixing height *h*:

- **Daytime**: The boundary layer deepens (h > 1000 m), diluting pollutants → low concentrations
- **Nighttime**: The boundary layer collapses (h < 100 m), trapping pollutants → high concentrations

This is far more important than the temperature effect added in Session 2, which barely changed the model output.
"""

# ╔═╡ c2d3e4f5-0003-0003-0003-000000000705
md"""
### 👉 Question 3: Model Evolution and Remaining Discrepancies

Looking at the comparison plot above, the Session 3 model captures the diurnal cycle much better than Sessions 1 and 2. However, there are still discrepancies between the Session 3 model and the observations.

Identify **two specific discrepancies** between the Session 3 model and the observations, and for each one, suggest a physical process or model improvement that could reduce the discrepancy.
"""

# ╔═╡ c2d3e4f5-0003-0003-0003-000000000706
hint(
    md"""
    Consider processes that occur in the real atmosphere but are not represented in our model.
    """
)

# ╔═╡ c2d3e4f5-0003-0003-0003-000000000707
answer_q3 = """
Enter your answer here.

""";

# ╔═╡ c2d3e4f5-0003-0003-0003-000000000800
md"""
## Summary

In this notebook we:

- Implemented **friction velocity** u\* and **boundary layer depth** zᵢ as a ModelingToolkit component with proper units — covering neutral (Eq. 7), stable (Eqs. 8–9), and unstable (Eq. 10) conditions

- Implemented the **Pasquill stability index** P (Eqs. 11–12) as a separate component

- Wrote **coupling equations** connecting BLD to the heat flux and box model, making the mixing height *h* vary diurnally

- Observed that Mumbai's **lower Coriolis parameter** (19°N vs 51°N) gives a deeper neutral BLD — better daytime ventilation

- Compared **three model versions** against observations, showing that the time-varying BLD is the key driver of the diurnal NO₂ cycle

**Key insight:** The boundary layer depth varies by more than an order of magnitude between day and night. This single parameter dominates the diurnal cycle of urban air pollution.

**Note:** The mixing layer depth zᵢ that we calculated here is not the best measure of box height for the box model. In the next session, we will develop a more physically appropriate method for determining the effective box height.

**Next:** Notebook 4 — Dispersion Parameter and Box Height
"""

# ╔═╡ c2d3e4f5-0003-0003-0003-000000000900
md"""
## Appendix: Components from Session 2

The following components and coupling equations were developed in Session 2. They are used throughout this notebook.
"""

# ╔═╡ c2d3e4f5-0003-0003-0003-000000000102
begin
    domain = DomainInfo(
...
# ╔═╡ c2d3e4f5-0003-0003-0003-000000000402
begin
	# Couple BoundaryLayerDepth ↔ BoxModelNO2
	...
	# Couple BoundaryLayerDepth ↔ SensibleHeatFlux
	...
	# Couple StabilityIndex ↔ SensibleHeatFlux
...
# ╔═╡ c2d3e4f5-0003-0003-0003-000000000405
let
    try
        # Test all three couplings by coupling all relevant components
        test_coupled = couple(
            BoxModelNO2(), SolarRadiation(domain), CloudCorrection(),
            SensibleHeatFlux(), BoundaryLayerDepth(), StabilityIndex()
        )
        test_sys = convert(System, test_coupled)
        prob = ODEProblem(test_sys, [], (0.0, 86400.0))
        sol = solve(prob, Tsit5(); saveat = 3600.0)

        zi_vals = sol[test_sys.BoundaryLayerDepth₊zi]
        C_vals = sol[test_sys.BoxModelNO2₊C]
        P_vals = sol[test_sys.StabilityIndex₊P]

        # Verify that coupling produces expected behavior:
        # - zi should have a strong diurnal cycle (BLD ↔ SensibleHeatFlux coupling)
        # - P should vary significantly (StabilityIndex ↔ SensibleHeatFlux coupling)
        # - C should respond to zi changes (BoxModelNO2 ↔ BLD coupling)
        zi_range = maximum(zi_vals) - minimum(zi_vals)
        P_range = maximum(P_vals) - minimum(P_vals)
        C_range = maximum(C_vals) - minimum(C_vals)

        if zi_range < 300
            keep_working(md"Boundary layer depth zᵢ does not show expected diurnal variation. Check the BLD ↔ SensibleHeatFlux coupling.")
        elseif P_range < 1.5
            keep_working(md"Stability index P does not vary as expected. Check the StabilityIndex ↔ SensibleHeatFlux coupling.")
        elseif C_range < 0.3
            keep_working(md"NO₂ concentration does not respond to boundary layer changes. Check the BoxModelNO2 ↔ BLD coupling.")
        elseif !(maximum(zi_vals) ≈ 3001.5707406641714)
            keep_working(md"Boundary layer depth values don't match expected results. Check all three coupling equations.")
        else
            correct(md"All three couplings work! zᵢ ranges from $(round(minimum(zi_vals), digits=0)) to $(round(maximum(zi_vals), digits=0)) m over 24 hours.")
        end
    catch e
        if e isa ModelingToolkitBase.ValidationError
            keep_working(md"The system contains a unit mismatch. This can happen if your equations are incorrect, or if you haven't yet replaced the placeholder values.")
...
# ╔═╡ c2d3e4f5-0003-0003-0003-000000000504
begin
    full3_sys = convert(System, full3_coupled)
    full3_prob = ODEProblem(
        full3_sys,
        [],
        get_tspan(domain)
    )
    full3_sol = solve(full3_prob, Tsit5(), saveat = 3600)
end

# ╔═╡ c2d3e4f5-0003-0003-0003-000000000510
let
    try
        zi_vals = full3_sol[full3_sys.BoundaryLayerDepth₊zi]
        C_vals = full3_sol[full3_sys.BoxModelNO2₊C]

        zi_range = maximum(zi_vals) - minimum(zi_vals)
        C_range = maximum(C_vals) - minimum(C_vals)

        if zi_range < 300
            keep_working(md"Boundary layer depth zᵢ does not show expected diurnal variation. Check that you've included all 7 components in `couple()`.")
        elseif C_range < 0.3
            keep_working(md"NO₂ concentration does not respond to boundary layer changes. Check that you've included all 7 components.")
        elseif !(maximum(zi_vals) ≈ 3002.106231139122)
            keep_working(md"Model output doesn't match expected values. Check that you've included all 7 components: `BoxModelNO2()`, `SolarRadiation(domain)`, `CloudCorrection()`, `SensibleHeatFlux()`, `BoundaryLayerDepth()`, `StabilityIndex()`, and `OpenAQ`.")
        else
            correct(md"Full 7-component model works! Maximum zᵢ = $(round(maximum(zi_vals), digits=0)) m, NO₂ varies from $(round(minimum(C_vals), digits=2)) to $(round(maximum(C_vals), digits=2)) μg/m³.")
        end
    catch e
        if e isa ModelingToolkitBase.ValidationError
            keep_working(md"The system contains a unit mismatch. This can happen if your equations are incorrect, or if you haven't yet replaced the placeholder values.")
...
# ╔═╡ c2d3e4f5-0003-0003-0003-000000000601
let
    # Build and solve a 24-hour simulation for the selected city
    lat = lat_selector
    u_val = u_bl_slider

    coupled_24h = couple(
        SolarRadiation(domain), CloudCorrection(), SensibleHeatFlux(),
        BoundaryLayerDepth(), StabilityIndex()
    )
    sys_24h = convert(System, coupled_24h)
    prob_24h = ODEProblem(
        sys_24h, [
            sys_24h.BoundaryLayerDepth₊lat => lat,
            sys_24h.SensibleHeatFlux₊u => u_val,
            sys_24h.SolarRadiation₊lat => lat,
        ], (0.0, 86400.0),
    )
    sol_24h = solve(prob_24h, Tsit5(); saveat = 900.0)

    zi_vals = sol_24h[sys_24h.BoundaryLayerDepth₊zi]
    zn_vals = sol_24h[sys_24h.BoundaryLayerDepth₊zn]
    P_vals = sol_24h[sys_24h.StabilityIndex₊P]
    hours_ist = sol_24h.t ./ 3600 .+ 5.5

    fig = Figure(size = (800, 550))

    ax1 = Axis(
        fig[1, 1], ylabel = "Boundary layer depth zᵢ (m)",
        title = "Diurnal zᵢ and P — $(lat == 19.0 ? "Mumbai" : "London") (u=$(u_val) m/s)",
        xticklabelsvisible = false, xticks = 0:3:24
    )
    lines!(ax1, hours_ist, zi_vals, color = :steelblue, linewidth = 2, label = "zᵢ")
    lines!(ax1, hours_ist, zn_vals, color = :gray, linewidth = 1, linestyle = :dot, label = "zₙ (neutral)")
    hlines!(ax1, [100.0], color = :firebrick, linestyle = :dash, label = "100 m threshold")
    axislegend(ax1, position = :lt)

    ax2 = Axis(
        fig[2, 1], xlabel = "Hour of day (IST)", ylabel = "Stability index P",
        xticks = 0:3:24, yticks = 1:7, limits = (nothing, (1, 7))
    )
    lines!(ax2, hours_ist, clamp.(P_vals, 1.0, 7.0), color = :darkorange, linewidth = 2)
    hlines!(ax2, [3.6], color = :black, linestyle = :dash)
...
# ╔═╡ c2d3e4f5-0003-0003-0003-000000000701
begin
    # Session 1: Box model only + OpenAQ
    s1_coupled = couple(
        BoxModelNO2(),
        OpenAQ("no2", domain; api_key = get(ENV, "OPENAQ_API_KEY", "")),
    )
    s1_sys = convert(System, s1_coupled)
    s1_prob = ODEProblem(s1_sys, [], get_tspan(domain))
    s1_sol = solve(s1_prob, Tsit5(), saveat = 3600)
end;

# ╔═╡ c2d3e4f5-0003-0003-0003-000000000702
begin
    # Session 2: Box model + heat flux (constant h) + OpenAQ
    s2_coupled = couple(
        BoxModelNO2(),
    ...
    s2_sys = convert(System, s2_coupled)
    s2_prob = ODEProblem(
        s2_sys,
        [],
        get_tspan(domain)
    )
    s2_sol = solve(s2_prob, Tsit5(), saveat = 3600)
end;

# ╔═╡ c2d3e4f5-0003-0003-0003-000000000703
let
    ugm3perkgm3 = 1.0e9
    ...
    # Observations
    obs_no2 = full3_sol[full3_sys.OpenAQ₊no2] .* ugm3perkgm3
    obs_no2 = [v != 0.0 ? v : missing for v in obs_no2]

    # Session 1 model
    m1 = s1_sol[s1_sys.BoxModelNO2₊C / s1_sys.BoxModelNO2₊χ] .* ugm3perkgm3
    # Session 2 model
    m2 = s2_sol[s2_sys.BoxModelNO2₊C / s2_sys.BoxModelNO2₊χ] .* ugm3perkgm3
    # Session 3 model (this notebook)
    m3 = full3_sol[full3_sys.BoxModelNO2₊C / full3_sys.BoxModelNO2₊χ] .* ugm3perkgm3

    # Boundary layer depth (Session 3 only)
    zi = full3_sol[full3_sys.BoundaryLayerDepth₊zi]

    fig = Figure(size = (900, 600))
    ax1 = Axis(
        fig[1, 1],
        ylabel = "NO₂ (μg m⁻³)",
        title = "Model Evolution: Sessions 1–3 vs. OpenAQ — Mumbai, Nov 15–22 2021",
        xticklabelsvisible = false
    )

    lines!(
        ax1, to_ist(full3_sol), obs_no2, color = :steelblue, linewidth = 2.5,
        label = "OpenAQ observations"
    )
    lines!(
        ax1, to_ist(s1_sol), m1, color = :gray60, linewidth = 1.5, linestyle = :dash,
        label = "Session 1 (box only)"
    )
    lines!(
        ax1, to_ist(s2_sol), m2, color = :darkorange, linewidth = 1.5, linestyle = :dashdot,
        label = "Session 2 (+ heat flux)"
    )
    lines!(
        ax1, to_ist(full3_sol), m3, color = :firebrick, linewidth = 2,
        label = "Session 3 (+ BLD)"
    )
    ...
    ax2 = Axis(
        fig[2, 1],
        xlabel = "Date (IST)",
        ylabel = "Boundary layer depth zᵢ (m)"
    )
    lines!(ax2, to_ist(full3_sol), zi, color = :firebrick, linewidth = 2)
...
manifest_format = "2.0"
project_hash = "47bc9bcd40b30791f151d87853586a6874dcf45a"
...
git-tree-sha1 = "20fb57046d6153827eb5436d4004b2840bb5407a"
repo-rev = "main"
