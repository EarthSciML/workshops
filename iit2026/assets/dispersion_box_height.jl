...
# ╔═╡ d3e4f5a6-0004-0004-0004-000000000001
begin
...
# ╔═╡ d3e4f5a6-0004-0004-0004-000000000099
@register_unit ppb 1

# ╔═╡ d3e4f5a6-0004-0004-0004-000000000003
md"""
# Session 4: Dispersion Parameter and Box Height

**Reference**: Middleton (1998), Sections 7–9

---

In Session 3 we made the boundary layer depth zᵢ vary diurnally — deep during the day (strong mixing, low pollution) and shallow at night (poor mixing, high pollution). But we used zᵢ directly as the box height *h*. In BOXURB, the **effective box height** is instead derived from the **vertical dispersion parameter** σz, which depends on atmospheric stability, city size, and surface roughness.
"""

# ╔═╡ d3e4f5a6-0004-0004-0004-000000000004
md"""
## Learning Objectives

After completing this notebook, you will be able to:

- Implement the **vertical dispersion parameter** σz (Eqs. 13–23) as a ModelingToolkit component with physical units
- Implement the **BOXURB box height** h = α·σz (Eq. 24) as a ModelingToolkit component, constrained by the boundary layer depth zᵢ
- Write **coupling equations** connecting the BoxHeight component to the existing model
- Compare **three successive model versions** (Sessions 2–4) against OpenAQ observations
"""

# ╔═╡ d3e4f5a6-0004-0004-0004-000000000005
md"""
The model components from Sessions 2–3 (BoxModelNO2, SolarRadiation, CloudCorrection, SensibleHeatFlux, BoundaryLayerDepth, StabilityIndex, and their coupling equations) are loaded in the **Appendix** at the end of this notebook.
"""

# ╔═╡ d3e4f5a6-0004-0004-0004-000000000006
md"""
## Recap: What We Built in Session 3

In Session 3, we implemented two key components:

1. **BoundaryLayerDepth** — calculates the boundary layer depth zᵢ from friction velocity u\*, Coriolis parameter f, and sensible heat flux H:
   - Neutral: ``z_n = 0.25\,u_*/f``
   - Stable (H < 0): ``z_i = (z_n^3 \, z_s^3 / (z_n^3 + z_s^3))^{1/3}``
   - Unstable (H > 0): ``z_i = \sqrt{z_n^2 + 1400\,S_i/(\rho c_p)}``

2. **StabilityIndex** — computes the Pasquill stability index P from heat flux and wind speed (Eqs. 11–12)

We used zᵢ directly as the box height h. But as Middleton (1998) notes in Section 8, the **effective box height** should be derived from the vertical dispersion parameter σz:

```math
h = \alpha \, \sigma_z(x, P, z_0), \quad \alpha = 1.25
```

The factor α = 1.25 comes from treating the vertical concentration profile as approximately Gaussian. Within 1.25 standard deviations of the surface, roughly 79% of the pollutant mass is contained, making this an effective mixing height for the box model.

The box height is constrained so that ``h \le z_i``. This is what we implement in this session.
"""

# ╔═╡ d3e4f5a6-0004-0004-0004-000000000100
md"""
## 1. Vertical Dispersion Parameter σz (Eqs. 13–23)

The vertical dispersion parameter σz (m) describes how far pollutants spread vertically as they travel a distance x downwind. It depends on:
- **Distance x** (m): larger x → more time for mixing → larger σz. In the box model, x represents the upwind fetch — the distance over which emissions accumulate and pollutants disperse vertically as they travel downwind across the city.
- **Stability index P**: unstable conditions (low P) → faster mixing → larger σz
- **Surface roughness z₀**: rougher surface → more mechanical turbulence → larger σz

### Formulas

From stability P, compute K, L, M, N (Eqs. 13–16):
```math
K = 0.165 - 0.017P, \quad L = 1.0 - 0.032P, \quad M = 0.77 - 0.022P, \quad N = 0.386 \times 10^{-3}P
```

From z₀ (metres): ``S_0 = \log_{10}(z_0)`` (Eq. 17)

From downwind distance x (metres): ``X = \log_{10}(x) - 3`` (Eq. 18)

Polynomial coefficients depending on X (Eqs. 19–22):
```math
A = 1.365 - 0.102X + 0.022X^2 + 0.05X^3
```
```math
B = 0.45 - 0.2738X + 0.0352X^2 + 0.0086X^3
```
```math
C_c = 0.075 - 0.074X + 0.019X^2 - 0.0005X^3
```
```math
D = 0.005 - 0.0045X + 0.0026X^2 - 0.0031X^3
```

Full dispersion formula (Eq. 23):
```math
\sigma_z(x, P, z_0) = \frac{K x^L}{1 + N x^M}(A + B S_0 + C_c S_0^2 + D S_0^3)
```

### Implementing as a ModelingToolkit component

Unlike Sessions 1–3 where we already had continuous differential equations, the σz formula is a **static algebraic relation** — it maps (x, P, z₀) to σz at each instant. We will express every sub-equation as an explicit MTK equation so that unit checking is preserved end-to-end.

**Nondimensionalization pattern:** Because `log10` and irrational exponents require dimensionless arguments, we divide physical quantities by **reference values** with the same units (e.g., `x / x_ref` where both are in metres). The final result is multiplied by a reference length `z_ref` to restore the unit of metres.
"""

# ╔═╡ d3e4f5a6-0004-0004-0004-000000000102
# Plain Julia helper for plotting only (not registered with MTK).
function _sigma_z_plot(x_m, P, z0_m)
    x_m = max(x_m, 1.0)
    z0_m = max(z0_m, 0.001)
    K = 0.165 - 0.017 * P
    L = 1.0 - 0.032 * P
    ...
    S0 = log10(z0_m)
    X = log10(x_m) - 3.0
    A = 1.365 - 0.102 * X + 0.022 * X^2 + 0.05 * X^3
    B = 0.45 - 0.2738 * X + 0.0352 * X^2 + 0.0086 * X^3
    Cc = 0.075 - 0.074 * X + 0.019 * X^2 - 0.0005 * X^3
    Dd = 0.005 - 0.0045 * X + 0.0026 * X^2 - 0.0031 * X^3
    return K * x_m^L / (1.0 + N * x_m^M) * (A + B * S0 + Cc * S0^2 + Dd * S0^3)
end;

# ╔═╡ d3e4f5a6-0004-0004-0004-000000000103
md"""
### σz curves for all Pasquill stability classes

The plot below shows σz vs. downwind distance for all seven Pasquill stability classes at z₀ = 0.3 m (suburban). Class E (slightly stable, P ≈ 4.3) is sometimes omitted in simplified schemes and interpolated between classes D and F; the σz formulas from Middleton (1998) apply continuously across all P values, so class E is obtained directly by evaluating at P = 4.3.
"""

# ╔═╡ d3e4f5a6-0004-0004-0004-000000000104
let
	function _sigma_z_plot(x_m, P, z0_m)
	    x_m = max(x_m, 1.0)
	    z0_m = max(z0_m, 0.001)
    ...
    x_range = 1.0e3:500:3.0e4   # 1 km to 30 km
    P_vals = [1.5, 2.5, 3.0, 3.6, 4.3, 5.0, 6.5]
    labels = ["A (very unstable)", "B", "C", "D (neutral)", "E (slightly stable)", "F", "G (very stable)"]
    colors = [:darkblue, :steelblue, :dodgerblue, :gray50, :goldenrod, :darkorange, :firebrick]

    fig = Figure(size = (750, 450))
    ax = Axis(
        fig[1, 1], xlabel = "Downwind distance x (km)", ylabel = "σz (m)",
        title = "Vertical dispersion parameter σz — suburban surface (z₀ = 0.3 m)",
        xscale = log10, yscale = log10,
        xticks = ([1.0e3, 2.0e3, 5.0e3, 1.0e4, 2.0e4, 3.0e4], ["1", "2", "5", "10", "20", "30"])
    )

    for (P_val, lbl, col) in zip(P_vals, labels, colors)
        sz = [_sigma_z_plot(x, P_val, 0.3) for x in x_range]
        lines!(ax, collect(x_range), sz, color = col, linewidth = 2, label = lbl)
    end
    hlines!(ax, [400.0], color = :black, linestyle = :dash, label = "Typical zi = 400 m")
    axislegend(ax, position = :lt, labelsize = 11)
    fig
end

# ╔═╡ d3e4f5a6-0004-0004-0004-000000000105
md"""
### 👉 Question 1: Reading the σz chart

Look at the chart above. For Mumbai's typical daytime conditions — **unstable** stability (P ≈ 2.5, class B) with a city fetch of **20 km** and suburban roughness (z₀ = 0.3 m):

1. What is the approximate value of σz?
2. What would the BOXURB box height be (h = 1.25·σz)?
3. If the boundary layer depth zᵢ is 800 m, is h constrained by zᵢ?
"""

# ╔═╡ d3e4f5a6-0004-0004-0004-000000000106
answer_q1 = """
Enter your answer here.
""";

# ╔═╡ d3e4f5a6-0004-0004-0004-000000000200
md"""
## 2. Box Height Component (Eqs. 13–24)

### 📟 Exercise 1: Implement the BoxHeight component

The BOXURB box height combines σz with the boundary layer depth constraint:

```math
h = \min\!\big(\alpha \, \sigma_z(x, P, z_0),\; z_i\big), \quad \alpha = 1.25
```

All intermediate quantities of the σz formula (Eqs. 13–23) are expressed as explicit MTK equations. Since `log10` and irrational exponents require dimensionless arguments, we divide by reference values (`x_ref`, `z_ref`) with the same units.

**Your task**: Replace each `missing` in the `BoxHeight` component below with the correct expression. There are **6 equations** to fill in:
- K, L, M, N (Eqs. 13–16, from stability P)
- σz (Eq. 23, combining all intermediates)
- h (Eq. 24, min constraint)
"""

# ╔═╡ d3e4f5a6-0004-0004-0004-000000000202
begin
	struct BoxHeightCoupler; sys; end
	@component function BoxHeight(; name=:BoxHeight)
		...
		eqs = [
			...
			# Stability coefficients (Eqs. 13-16) — FILL IN
			K_coeff ~ missing,  # Eq. 13
			L_coeff ~ missing,  # Eq. 14
			M_coeff ~ missing,  # Eq. 15
			N_coeff ~ missing,  # Eq. 16
			# Log-transformed inputs (Eqs. 17-18)
			S0 ~ log10(z0_nd),
			...
			# Dispersion parameter (Eq. 23) — FILL IN
			σz ~ missing,
			# Box height (Eq. 24) — FILL IN
			h ~ missing,
		]
		System(eqs, t; name,
...
# ╔═╡ d3e4f5a6-0004-0004-0004-000000000204
details(
...
# ╔═╡ d3e4f5a6-0004-0004-0004-000000000205
let
    if !@isdefined(BoxHeight)
        not_defined(:BoxHeight)
    else
        try
            bh = BoxHeight()
            sys = mtkcompile(bh)
            prob = ODEProblem(
                sys,
                [sys.P => 3.6, sys.x => 20000.0, sys.z0 => 0.3, sys.zi => 800.0],
                (0.0, 1.0)
            )
            sol = solve(prob, Tsit5())
            sz_val = sol[sys.σz][1]
            h_val = sol[sys.h][1]
            sz_ref = _sigma_z_plot(20000.0, 3.6, 0.3)
            h_ref = min(1.25 * sz_ref, 800.0)
            if ismissing(sz_val)
                still_missing(md"Fill in the 6 equations in the BoxHeight component.")
            elseif !isfinite(sz_val) || sz_val <= 0
                keep_working(md"σz returned a non-positive or non-finite value. Check the K, L, M, N coefficients and the σz equation.")
            elseif !(sz_val ≈ sz_ref)
                keep_working(md"σ_z does not match the expected value. Check your K, L, M, N coefficients and the σ_z formula.")
            elseif !(h_val ≈ h_ref)
                keep_working(md"Box height h does not match the expected value. Check your h equation min constraint.")
            else
                correct(md"BoxHeight works! σz = $(round(sz_val, digits=0)) m, h = $(round(h_val, digits=0)) m.")
            end
        catch e
...
# ╔═╡ d3e4f5a6-0004-0004-0004-000000000206
md"""
### Interactive: Box height h

Use the sliders to explore how atmospheric stability, city size, and boundary layer depth affect the BOXURB box height h.

- Move **P** from unstable (1.5) to stable (6.5) and watch h change by more than an order of magnitude.
- Try **Mumbai city fetch** of 15–25 km (the Greater Mumbai urban agglomeration spans roughly 20 km NW–SE).
- The **zᵢ** (boundary layer depth) cap matters most during the early afternoon on clear days — try zᵢ = 1200–1800 m for a Mumbai winter day under high insolation.
"""

# ╔═╡ d3e4f5a6-0004-0004-0004-000000000207
tip(
    md"""
    **Mumbai stability context:** Mumbai's tropical latitude (~19°N) means high solar irradiance year-round.
    On a clear January day, daytime stability is typically **P ≈ 1.5–2.5** (class A–B, very unstable to unstable).
    Under sea-breeze conditions (afternoon westerlies) wind speeds of 3–5 m/s shift P toward **B–C (P ≈ 2.5–3.0)**.
    Neutral conditions (P ≈ 3.6) only occur around sunrise/sunset or under overcast skies.
    Stable conditions (P > 4) occur at night, especially in the pre-monsoon dry season.
    """
)

# ╔═╡ d3e4f5a6-0004-0004-0004-000000000208
md"""
| Parameter | Value | Notes |
| ---- | ---- | ---- |
| Stability P: | $(@bind P_slider PlutoUI.Slider(1.0:0.5:7.0, default=3.6, show_value=true)) | 1 = very unstable, 7 = very stable |
| City downwind size x (km): | $(@bind x_city_slider PlutoUI.Slider(1:1:30, default=20, show_value=true)) | Mumbai urban area ≈ 15–25 km |
| Boundary layer depth zi (m): | $(@bind zi_slider PlutoUI.Slider(100:50:2000, default=800, show_value=true)) | Mumbai winter daytime: 800–1500 m; nighttime: 100–200 m |
"""

# ╔═╡ d3e4f5a6-0004-0004-0004-000000000209
let
    x_m = x_city_slider * 1000.0
    sz = _sigma_z_plot(x_m, P_slider, 0.3)
    h_unconstrained = 1.25 * sz
    h_final = min(h_unconstrained, zi_slider)
    constrained = h_unconstrained > zi_slider
    md"""
    σz = **$(round(sz, digits=0)) m** |
    h = 1.25·σz = **$(round(h_unconstrained, digits=0)) m**
    $(constrained ? "→ **constrained to zᵢ = $(zi_slider) m**" : "(unconstrained, zᵢ not limiting)")

    **Final box height h = $(round(h_final, digits=0)) m**
    """
end

# ╔═╡ d3e4f5a6-0004-0004-0004-000000000400
md"""
## 3. Coupling the New Components

### 📟 Exercise 2: Write the coupling equations

We need coupling equations to connect the BoxHeight component to the existing model:

1. **BoxHeight ↔ BoxModelNO2**: The box model's mixing height `h` should come from the BoxHeight component. Convert `h` in BoxModelNO2 to a variable and connect `bm.h` to `bh.h`.

2. **BoxHeight ↔ BoundaryLayerDepth**: BoxHeight needs `zi` from the BoundaryLayerDepth. Convert `zi` in BoxHeight to a variable and connect `bh.zi` to `bl.zi`.

3. **BoxHeight ↔ StabilityIndex**: BoxHeight needs `P` from StabilityIndex. Convert `P` in BoxHeight to a variable and connect `bh.P` to `si.P`.

4. **BoxHeight ↔ BoxModelNO2 (x)**: BoxHeight needs city dimension `x`. Convert `x` in BoxHeight to a variable and connect `bh.x` to `bm.x`.

**Your task**: Replace each `missing` in the coupling equations below.
"""

# ╔═╡ d3e4f5a6-0004-0004-0004-000000000404
details(
    "▶ Reference implementation (reveal after attempting)", Markdown.parse(
        """
        ```julia
        # BoxHeight ↔ BoxModelNO2: box model gets h, BoxHeight gets x
        function EarthSciMLBase.couple2(bm::BoxModelNO2Coupler, bh::BoxHeightCoupler)
            bm, bh = bm.sys, bh.sys
            bm = param_to_var(bm, :h)
            bh = param_to_var(bh, :x)
            ConnectorSystem([bm.h ~ bh.h, bh.x ~ bm.x], bm, bh)
        end

        # BoxHeight ↔ BoundaryLayerDepth: BoxHeight gets zi
        function EarthSciMLBase.couple2(bh::BoxHeightCoupler, bl::BoundaryLayerDepthCoupler)
            bh, bl = bh.sys, bl.sys
            bh = param_to_var(bh, :zi)
            ConnectorSystem([bh.zi ~ bl.zi], bh, bl)
        end

        # BoxHeight ↔ StabilityIndex: BoxHeight gets P
        function EarthSciMLBase.couple2(bh::BoxHeightCoupler, si::StabilityIndexCoupler)
            bh, si = bh.sys, si.sys
            bh = param_to_var(bh, :P)
            ConnectorSystem([bh.P ~ si.P], bh, si)
        end
        ```
        """
    )
...
# ╔═╡ d3e4f5a6-0004-0004-0004-000000000500
md"""
## 4. Full Coupled Model

### 📟 Exercise 3: Couple the full model and solve

**Your task**: Use `couple()` to combine all eight components — `BoxModelNO2()`, `SolarRadiation()`, `CloudCorrection()`, `SensibleHeatFlux()`, `BoundaryLayerDepth()`, `StabilityIndex()`, `BoxHeight()`, and the OpenAQ observations — into a single coupled system. Then convert, build the ODE problem, and solve.
"""

# ╔═╡ d3e4f5a6-0004-0004-0004-000000000502
full4_coupled = couple(
	missing # Exercise 3: couple all eight components
)

# ╔═╡ a736ef19-a359-4ee2-9dce-70ae0f15b6e8
md"This is what our coupled model looks like now:"

# ╔═╡ c33ad0f0-3a82-4609-ae21-c9771a163c0b
let
	g = graph(full4_coupled)
...
# ╔═╡ 4eda1ec2-efff-4cec-a5c8-c97746839875
md"And now we can run a simulation:"

# ╔═╡ d3e4f5a6-0004-0004-0004-000000000505
nothing

# ╔═╡ d3e4f5a6-0004-0004-0004-000000000506
md"""
### Interactive: Diurnal box height

Use the wind speed slider to explore how the diurnal cycle of box height changes.
"""

# ╔═╡ d3e4f5a6-0004-0004-0004-000000000507
md"""
| Parameter | Control | Units |
|:---|:---|:---:|
| Wind speed u: | $(@bind u_s4_slider PlutoUI.Slider(1.0:0.5:10.0, default=3.0, show_value=true)) | m/s |
"""

# ╔═╡ d3e4f5a6-0004-0004-0004-000000000300
md"""
## 5. Recap: Urban Heat Store

In Sessions 2–3, we integrated the **urban heat store** (Middleton 1998, Eqs. 25–28) directly into the `SensibleHeatFlux` component. Recall that the component now computes:

- **Rural H**: the sensible heat flux from Eqs. 4–5 (daytime solar-driven, nighttime radiative cooling)
- **ΔQ\_H**: the urban heat store correction — an ODE that accumulates ``f_s`` times the excess solar flux during the day and decays exponentially with timescale ``\tau_{hs}``
- **H\_urban**: the total urban flux ``H + \Delta Q_H + \Delta Q_A``
...
The `StabilityIndex` now uses `H_urban` (not rural `H`) for computing the Pasquill stability class, so the urban heat store affects dispersion through the stability → σz → box height pathway. This delays the evening stability transition by 1–3 hours in cities compared to the countryside.
"""

# ╔═╡ d3e4f5a6-0004-0004-0004-000000000307
md"""
### 👉 Question 2: Urban heat store and evening pollution
...
1. Why does the city-centre urban heat flux stay positive longer than the suburban case?
...
# ╔═╡ d3e4f5a6-0004-0004-0004-000000000308
answer_q2 = """
Enter your answer here.
""";

# ╔═╡ d3e4f5a6-0004-0004-0004-000000000600
md"""
## 6. Model Comparison: Sessions 2–4 vs. Observations

Now let's compare model versions, all against **OpenAQ** NO₂ observations for Mumbai, Nov 15–22, 2021:

1. **Session 2** — Box model + sensible heat flux (constant h = 400 m, varying T)
2. **Session 4 without urban heat** — Full Session 4 model (BoxHeight + BLD + StabilityIndex) but with ΔQ\_A = 0 and f\_s = 0 (no anthropogenic heat flux or urban heat store)
3. **Session 4 full** — Full Session 4 model with ΔQ\_A = 6 W/m² and f\_s = 0.046 (anthropogenic heat from vehicles, buildings, and industry)

All three models use the Session 4 BoxHeight approach (h = min(1.25·σz, zᵢ)) except for Session 2, which uses constant h = 400 m. Comparing models 1 and 2 shows the effect of dispersion-based box height; comparing 2 and 3 isolates the effect of anthropogenic heat.
"""

# ╔═╡ d3e4f5a6-0004-0004-0004-000000000604
md"""
The comparison shows two effects:
- **Dispersion-based box height** (Session 2 → Session 4 without urban heat): The time-varying h = min(1.25·σz, zᵢ) captures the strong diurnal cycle that the constant h = 400 m completely missed
- **Anthropogenic heat** (Session 4 without urban heat → Session 4 full): The extra anthropogenic heat (ΔQ\_A = 6 W/m²) and urban heat store make conditions slightly more unstable, affecting the boundary layer depth and timing of pollution peaks
"""

# ╔═╡ d3e4f5a6-0004-0004-0004-000000000605
md"""
### 👉 Question 3: Model Improvement and Remaining Discrepancies

Looking at the comparison plot above:

1. What is the main effect of adding **anthropogenic heat** ΔQ\_A (comparing the orange and red curves)?
2. Does this improve or worsen agreement with observations, and for which part of the day?
3. Identify one remaining discrepancy between the Session 4 model and observations, and suggest a change to the model that could reduce it.
"""

# ╔═╡ d3e4f5a6-0004-0004-0004-000000000606
answer_q3 = """
Enter your answer here.
""";

# ╔═╡ d3e4f5a6-0004-0004-0004-000000000700
md"""
## Summary

In this notebook we:

- Implemented the **vertical dispersion parameter** σz (Eqs. 13–23) as inline ModelingToolkit equations in the **BoxHeight** component (Eq. 24), giving a more physically appropriate effective box height h = 1.25·σz constrained by zᵢ

- Wrote **coupling equations** connecting BoxHeight to the existing model components from Sessions 2–3

- Compared **three successive model versions** (Sessions 2–4) against OpenAQ observations, showing progressive improvement in capturing the diurnal NO₂ cycle

**Key insight:** The effective box height from σz is typically smaller than the full boundary layer depth during the day, which significantly impacts predicted pollution concentrations.

**Next:** Notebook 5 — Emissions and the Full BOXURB Model
"""

# ╔═╡ d3e4f5a6-0004-0004-0004-000000000800
md"""
## Appendix: Components from Sessions 2–3

The following components and coupling equations were developed in Sessions 2–3. They are used throughout this notebook.
"""

# ╔═╡ d3e4f5a6-0004-0004-0004-000000000801
begin
    domain = DomainInfo(
...
# ╔═╡ d3e4f5a6-0004-0004-0004-000000000402
begin
...
# ╔═╡ d3e4f5a6-0004-0004-0004-000000000405
let
	execution_tracker
    # Reference values computed from the correct coupled model solution
    h_max_ref = 2751.9407804444936 # maximum box height over 24 hours (m)
    h_min_ref = 462.0439737478587   # minimum box height over 24 hours (m)
    try
        test_coupled = couple(
            SolarRadiation(domain), CloudCorrection(), SensibleHeatFlux(),
            BoundaryLayerDepth(), StabilityIndex(), BoxHeight()
        )
        test_sys = convert(System, test_coupled)
        prob = ODEProblem(test_sys, [], (0.0, 86400.0))
        sol = solve(prob, Tsit5(); saveat = 3600.0)
        h_vals = sol[test_sys.BoxHeight₊h]
        if !(maximum(h_vals) ≈ h_max_ref)
            keep_working(md"Maximum box height does not match the expected value. Check your coupling equations connecting BoxHeight to BoundaryLayerDepth and StabilityIndex.")
        elseif !(minimum(h_vals) ≈ h_min_ref)
            keep_working(md"Minimum box height does not match the expected value. Check your coupling equations connecting BoxHeight to BoundaryLayerDepth and StabilityIndex.")
        else
            correct(md"Coupling works! Box height h ranges from $(round(minimum(h_vals), digits=0)) to $(round(maximum(h_vals), digits=0)) m over 24 hours.")
        end
    catch e
        if e isa ModelingToolkitBase.ValidationError
            keep_working(md"The system contains a unit mismatch. This can happen if your equations are incorrect, or if you haven't yet replaced the placeholder values.")
...
# ╔═╡ d3e4f5a6-0004-0004-0004-000000000504
begin
    full4_sys = convert(System, full4_coupled)
    full4_prob = ODEProblem(
        full4_sys,
        [],
        get_tspan(domain)
    )
    full4_sol = solve(full4_prob, Tsit5(), saveat = 3600)
end

# ╔═╡ d3e4f5a6-0004-0004-0004-000000000510
let
    # Reference values computed from the correct full 8-component coupled model solution
    h_max_ref = 2751.153818456176   # maximum box height over simulation (m)
    C_mean_ref = 5.996380521508489   # mean NO2 concentration (ppb)
    try
        h_vals = full4_sol[full4_sys.BoxHeight₊h]
        C_vals = full4_sol[full4_sys.BoxModelNO2₊C]
        if !all(isfinite, C_vals) || !all(>(0), C_vals)
            keep_working(md"Some NO₂ concentrations are non-finite or negative. Check that all 8 components are coupled correctly.")
        elseif !all(isfinite, h_vals) || !all(>(0), h_vals)
            keep_working(md"Some box height values are non-finite or negative. Check that all 8 components are coupled correctly.")
        elseif !(maximum(h_vals) ≈ h_max_ref)
            keep_working(md"Maximum box height does not match the expected value. Check that all 8 components are included in couple().")
        elseif !(sum(C_vals) / length(C_vals) ≈ C_mean_ref)
            keep_working(md"Mean NO₂ concentration does not match the expected value. Check that all 8 components are included in couple().")
        else
            correct(md"Full coupled model works! h ranges up to $(round(maximum(h_vals), digits=0)) m, mean NO₂ = $(round(sum(C_vals)/length(C_vals), digits=1)) ppb.")
        end
    catch e
        if e isa ModelingToolkitBase.ValidationError
            keep_working(md"The system contains a unit mismatch. This can happen if your equations are incorrect, or if you haven't yet replaced the placeholder values.")
...
# ╔═╡ d3e4f5a6-0004-0004-0004-000000000508
let
    coupled_24h = couple(
        SolarRadiation(domain), CloudCorrection(), SensibleHeatFlux(),
        BoundaryLayerDepth(), StabilityIndex(), BoxHeight()
    ...
    prob_24h = ODEProblem(
        sys_24h, [
            sys_24h.SensibleHeatFlux₊u => u_s4_slider,
        ], (0.0, 86400.0)
    )
    sol_24h = solve(prob_24h, Tsit5(); saveat = 900.0)

    h_vals = sol_24h[sys_24h.BoxHeight₊h]
    ...
    ax1 = Axis(
        fig[1, 1], xlabel = "Hour of day (IST)", ylabel = "Height (m)",
        title = "Diurnal box height and BLD — Mumbai (u=$(u_s4_slider) m/s)",
        xticks = 0:3:30
    )
    lines!(ax1, hours_ist, h_vals, color = :firebrick, linewidth = 2, label = "Box height h")
    lines!(ax1, hours_ist, zi_vals, color = :steelblue, linewidth = 1.5, linestyle = :dash, label = "BLD zᵢ")
    axislegend(ax1, position = :lt)

    fig
end

# ╔═╡ d3e4f5a6-0004-0004-0004-000000000601
begin
    ...
    s2_prob = ODEProblem(s2_sys, [], get_tspan(domain))
    s2_sol = solve(s2_prob, Tsit5(), saveat = 3600)
end;

# ╔═╡ d3e4f5a6-0004-0004-0004-000000000602
begin
    # Session 4 without urban heat: BLD + StabilityIndex + BoxHeight, with no urban heat
    # Setting ΔQ_A=0 and f_s=0 disables urban heat effects for the comparison.
    s4_no_urban_heat = couple(
        BoxModelNO2(),
        SolarRadiation(domain),
        CloudCorrection(),
        SensibleHeatFlux(),
    ...
    s4_no_urban_heat_sys = convert(System, s4_no_urban_heat)
    s4_no_urban_heat_prob = ODEProblem(
        s4_no_urban_heat_sys,
        [
            s4_no_urban_heat_sys.SensibleHeatFlux₊ΔQ_A => 0.0,
            s4_no_urban_heat_sys.SensibleHeatFlux₊f_s => 0.0,
        ],
        get_tspan(domain)
    )
    s4_no_urban_heat_sol = solve(s4_no_urban_heat_prob, Tsit5(), saveat = 3600)
end;

# ╔═╡ d3e4f5a6-0004-0004-0004-000000000603
let
    ugm3perkgm3 = 1.0e9
    ...
    # Observations
    obs_no2 = full4_sol[full4_sys.OpenAQ₊no2] .* ugm3perkgm3
    obs_no2 = [v != 0.0 ? v : missing for v in obs_no2]

    # Session 2 model
    m2 = s2_sol[s2_sys.BoxModelNO2₊C / s2_sys.BoxModelNO2₊χ] .* ugm3perkgm3
    # Session 4 without urban heat model
    m3 = s4_no_urban_heat_sol[s4_no_urban_heat_sys.BoxModelNO2₊C / s4_no_urban_heat_sys.BoxModelNO2₊χ] .* ugm3perkgm3
    # Session 4 full model (this notebook)
    m4 = full4_sol[full4_sys.BoxModelNO2₊C / full4_sys.BoxModelNO2₊χ] .* ugm3perkgm3

    # Box height
    h4 = full4_sol[full4_sys.BoxHeight₊h]

    fig = Figure(size = (900, 550))
    ax1 = Axis(
        fig[1, 1],
        ylabel = "NO₂ (μg m⁻³)",
        title = "Model Evolution: Sessions 2–4 vs. OpenAQ — Mumbai, Nov 15–22 2021",
        xticklabelsvisible = false
    )

    lines!(
        ax1, to_ist(full4_sol), obs_no2, color = :steelblue, linewidth = 2.5,
        label = "OpenAQ observations"
    )
    lines!(
        ax1, to_ist(s2_sol), m2, color = :gray60, linewidth = 1.5, linestyle = :dash,
        label = "Session 2 (const h=400 m)"
    )
    lines!(
        ax1, to_ist(s4_no_urban_heat_sol), m3, color = :darkorange, linewidth = 1.5, linestyle = :dashdot,
        label = "Session 4 w/o urban heat (ΔQ_A=0, f_s=0)"
    )
    lines!(
        ax1, to_ist(full4_sol), m4, color = :firebrick, linewidth = 2,
        label = "Session 4 full (ΔQ_A=6 W/m²)"
    )
    ...
    lines!(
        ax2, to_ist(full4_sol), h4, color = :firebrick, linewidth = 2,
        label = "Box height h"
    )
